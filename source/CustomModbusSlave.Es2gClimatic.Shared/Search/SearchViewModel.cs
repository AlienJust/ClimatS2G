	...
	public class SearchViewModel : ViewModelBase {
		...
		private void UpdateParamsVm() {
			...
			if (string.IsNullOrEmpty(_filterText)) {
				...
				foreach (var topLevelGroup in _rootGroups) {
					int paramsCount = 0;
					var copyOfDispGroup = FilterGroupsAndParamsRecurse(topLevelGroup, _filterText.ToLower(CultureInfo.CurrentCulture), new List<string>(), ref paramsCount);
		...
		private IDisplayGroup FilterGroupsAndParamsRecurse(IDisplayGroup dispGroup, string filterText, IEnumerable<string> parentNames, ref int paramsCount) {
			...
			foreach (var param in dispGroup.ParametersAndGroups) {
				if (param is IDisplayGroup group) {
					var copyOfSubgroup = FilterGroupsAndParamsRecurse(group, filterText, searchTags, ref paramsCount);
					if (copyOfSubgroup != null) {
						copyOfDispGroup.AddParameterOrGroup(copyOfSubgroup);
					}
				}
				else {
					if (searchTags.Any(tag => tag.ToLower(CultureInfo.CurrentCulture).Contains(_filterText)) || param.DisplayName.ToLower(CultureInfo.CurrentCulture).Contains(_filterText)) {
