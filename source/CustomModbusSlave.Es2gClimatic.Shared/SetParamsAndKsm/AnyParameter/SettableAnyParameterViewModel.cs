...
namespace CustomModbusSlave.Es2gClimatic.Shared.SetParamsAndKsm.AnyParameter {
	class SettableAnyParameterViewModel : ViewModelBase {
		...
		public BytesPair? UshortValue
		{
			...
			set
			{
				...
				else DoubleValue = _doubleBytesPairConverter.ConvertToSomething(value.Value);
			}
		}
		...
		public BytesPair? ReceivedUshortValue {
			...
			set {
				...
				else ReceivedDoubleValue = _doubleBytesPairConverter.ConvertToSomething(value.Value);
			}
		}
