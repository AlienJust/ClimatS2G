...
namespace CustomModbusSlave.Es2gClimatic.Shared.Ksm.WamOrCoolForcedModes {
	public class WarmOrCoolForcedModeViewModel : SettableDropdownParameterViewModel<BytesPair>, IReceivableParameter, ISettableBytesPairParameter {
		public WarmOrCoolForcedModeViewModel(int paramIndex, string name, BytesPair? defaultValue, IBuilderOneToOne<BytesPair, string> stringBuilder, List<IRawAndConvertedValues<BytesPair, string>> dropItems, string toolTipText, IParameterSetter parameterSetter, IThreadNotifier uiNotifier, IBuilderOneToOne<BytesPair, BytesPair> sendConverter) : base(paramIndex, name, defaultValue, stringBuilder, dropItems, toolTipText, parameterSetter, uiNotifier, sendConverter) {
		}
