	...
	internal sealed class TabInterfaceBuilder : IBuilderManyToOne<IDisplayGroup> {
		...
		public IDisplayGroup Build() {
			...
			*/
			...
			var setParam827 = new SettParamViewModel<int>("827: Ручной/Автоматический режим", _uiNotifier, val=> {if (val != 0 && val != 1)throw new ArgumentOutOfRangeException();}, -1, -1, (dspval, callback) =>_parameterSetter.SetParameterAsync(827, (ushort)dspval, ex=>callback(ex)));
			
			setParamsGroup.AddParameterOrGroup(setParam827);
