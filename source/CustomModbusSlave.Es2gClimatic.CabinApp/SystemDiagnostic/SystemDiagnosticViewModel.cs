...
namespace CustomModbusSlave.Es2gClimatic.CabinApp.SystemDiagnostic {
	class SystemDiagnosticViewModel : ViewModelBase {
		...
		private void CmdListenerMukWarmFloorReply03OnDataReceived(IList<byte> bytes, IMukWarmFloorReply03Data data) {
			_uiNotifier.Notify(() => {
				MukInfo6 = IsFullVersion ? new TextFormatterIntegerDotted().Format(data.FirmwareVersion) : OkLinkText;
				MukInfoColor6 = OkLinkColor;

				FanAirExhausterInfo = data.PwmHeat.ToString(CultureInfo.InvariantCulture);
				/*if (data.Diagnostic1.GetBit(5)) {
